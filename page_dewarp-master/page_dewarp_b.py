...
REMAP_DECIMATE = 2      # downscaling factor for remapping image
...
def remap_image(name, img, small, page_dims, params):
    ...
    page_x_range = np.linspace(0, float(page_dims[0]), int(width_small))
    page_y_range = np.linspace(0, float(page_dims[1]), int(height_small))
...
def main():
    ...
    print(file_list,os.path.abspath(__file__))
